# ST Chat Jumper

一个独立的 SillyTavern 前端插件，提供可拖拽的悬浮跳转条，用于快速跳转聊天楼层。

## 功能

<video src="https://github.com/user-attachments/assets/04cab1ea-01f4-4179-a2bd-625b945f051a.mp4" controls="controls" width="80%" loop></video>

- 快速跳转到**最近 3 楼**：按钮 `1 / 2 / 3`
  - `1` = 最新楼
  - `2` = 倒数第 2 楼
  - `3` = 倒数第 3 楼
- **上一楼 / 下一楼（头部）**：按钮 `<` / `>`（以当前屏幕中最“居中/可见”的消息作为基准）
- **当前楼层对齐**：
  - `H`：跳转到“当前楼层”的**头部**
  - `L`：跳转到“当前楼层”的**尾部**

- **显示楼层区间**：可输入楼层区间（如 `120-180`），只跳转并显示指定区间的楼层
  - 适合长聊天临时聚焦某段楼层
  - 区间生效后才会显示旋转箭头按钮，用于**恢复默认楼层**
  - 区间生效后，跳转栏会显示 `区间 x-y` 标签
  - 点击标签内的 `改` 可一键重新输入并切换到新范围

- **✏️ 快速编辑/定位**：
  - 若在聊天记录中**框选了文字**：点击铅笔会打开该楼层的编辑器，并尝试在编辑框中**高亮定位**到对应文本
  - 若没有框选文字：默认编辑**当前楼层**
  - 编辑中再次点击铅笔会自动点“完成”并回到进入编辑前的滚动位置  
  
- **临时收藏楼层（仅当前页面）**：按钮 `📌`
  - 点击 `📌` 进入“点选收藏”模式：鼠标变成小图钉，然后点击聊天中的目标楼层即可收藏/取消
  - `📌` 旁边的**小箭头**用于展开/收起收藏悬浮列表
  - 收藏悬浮列表：点击条目可跳转到该楼层**顶部**，右侧 `×` 可移除
  - 收藏仅在当前页面有效：刷新页面或切换聊天文件会自动清空
  - 点选模式可按 `ESC` 退出

- **横/竖布局**：按钮 `↔ / ↕` 用于切换
- **收起/展开**：小按钮 `– / +` 可将整条跳转栏收起
- **拖拽**：按住左侧/上方的拖拽手柄（细条）拖动，可移动位置



## 安装方法

- 自动安装
在酒馆的`安装拓展`界面输入以下url：
```
https://github.com/qianzhuowo/ST-Chat-Jumper
```

- 手动安装
把整个`ST Chat Jumper`文件夹放到：
```text
SillyTavern/public/scripts/extensions/third-party/
```
然后在 SillyTavern 的扩展管理里启用该插件。
